  ...
  render() {
    ...
    if (questionNo === 1) {
      buttonList = questionList[questionNo].display.map((option) => {
        return <button className="btn btn-lg btn-wide font-bold text-3xl" onClick={this.handleClickForward} key={option} value={option}>{option}</button>
      })
  
    } else if (questionNo === 2) {
      buttonList = questionList[questionNo].display[categorySelection].map((option) => {
        return <button className="btn btn-lg btn-wide font-bold text-3xl" onClick={this.handleClickForward} key={option} value={option}>{option}</button>
      })
    }

    //CLICK BACK BUTTON NOT WORKING ON THE PlaylistSelection 2ND PAGE.
    ...
    return (
      <div className="h-screen flex flex-col justify-center content-center items-center gap-4">
        ...
        <div className="flex justify-between content-center items-center gap-4">
          <button onClick={handlePrevious} value="back">Back</button>
          <button onClick={handleRestart} value="restart">Restart</button>
        </div>
      </div>
    );
  }
