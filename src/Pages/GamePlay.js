...
export default class GamePlay extends React.Component {
  ...
  // Select certian properties of an object
  ...
  validateAnswerEasy = (event) => {
    let { correctAnswer, handleUpdate, scoreCount, isPlaying, togglePlay } = this.props;
    ...
    if (event.target.value === correctAnswer.trackName) {
      ...
      this.setState({
        ...
        setTimeout(() => {this.songSelection()}, 2000)
      })
    ...
    if (Object.keys(selectedButtonValue).length === 3) {
      ...
      if (this.objectsComparison(correctAnswer, selectedButtonValue)) {
        ...
        this.setState({
          ...
          setTimeout(() => {this.songSelection()}, 3000)
        });
  ...
  render() {
    let { gameSelection, combinedTrack, scoreCount, handleNext, handleUpdate, isPlaying, togglePlay, audioRef } = this.props;
    ...
    if (gameSelection === "Easy") {
      ...
      if (selectedButtonValue === "") {
        ...
        selectionButtons = selectionButtons.map((buttonObj) => {
          if (buttonObj.props.value === selectedButtonValue) {
            ...
            return React.cloneElement(buttonObj, { className: "btn btn-lg btn-wide bg-green-500 normal-case" });
          } else {
            console.log(buttonObj.props.value);
            return React.cloneElement(buttonObj, { className: "btn btn-lg btn-wide btn-disabled normal-case" });
          }
        });
        ...
        selectionButtons = selectionButtons.map((buttonObj) => {
          if (buttonObj.props.value === selectedButtonValue) {
            console.log(buttonObj.props.value);
            return React.cloneElement(buttonObj, { className: "btn btn-lg btn-wide bg-red-500 normal-case" });
      ...
      // let nestedArrayTrack = combinedTrack.map((trackObj) => {
      //   return this.selectProperties(trackObj, ["trackName", "artistName", "albumName"]);
      // })
      // let flattenTrackDetails = [].concat(...nestedArrayTrack);
      // let shuffledTrackDetails = this.shuffleArray(flattenTrackDetails);

      // selectionButtons = shuffledTrackDetails.map((items) => {
      //   return <button onClick={this.validateAnswerIntermediate} value={items}>{items}</button>;
      // });
      ...
      let nestedKeyValuePairs = combinedTrack.map((trackObj) => (
        this.selectKeyValuePairs(trackObj, ["trackName", "artistName", "albumName"])
      ))
      ...
      if (result === "") {
        ...
        selectionButtons = selectionButtons.map((buttonObj) => {
          if (buttonObj.props.value === selectedButtonValue[buttonObj.props.name]) {
            return React.cloneElement(buttonObj, { className: "text-sm btn btn-md btn-wide bg-slate-400 normal-case" });
          }
          return buttonObj;
        });
        ...
        selectionButtons = selectionButtons.map((buttonObj) => {
          if (buttonObj.props.value === selectedButtonValue[buttonObj.props.name]) {
            return React.cloneElement(buttonObj, { className: "text-sm btn btn-md btn-wide bg-green-500 normal-case" });
          };
          return buttonObj;
        })
        announcement = "Congratz, You are correct!";
        ...
        selectionButtons = selectionButtons.map((buttonObj) => {
          ...
          if (buttonObj.props.value === selectedButtonValue[buttonObj.props.name]) {
            return React.cloneElement(buttonObj, { className: "text-sm btn btn-md btn-wide bg-red-500 normal-case" });
          }  
          return buttonObj;
        })
        announcement = "Wrong Answer!";
